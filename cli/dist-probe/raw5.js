...
for (let n = 0; n < 2; n++) {
  ...
  while (Date.now() < end) {
    const r = await Promise.race([reader.read(), new Promise((res) => setTimeout(() => res("T"), Math.max(1, end - Date.now())))]);
    if (r === "T") break;
    const v = r.value;
    if (v && v.length) got.push([...v].map((x) => x.toString(16).padStart(2, "0")).join(" "));
  }
  console.log(`attempt ${n}:`, got.length ? got.join(" | ") : "(nothing)");
}
await port.close();
