...
var req = new Uint8Array([184, 18, 241, 2, 26, 128, 195]);
var t0 = Date.now();
var pending = reader.read();
await port.setSignals({ dataTerminalReady: true });
await writer.write(req);
await new Promise((r2) => setTimeout(r2, 9));
await port.setSignals({ dataTerminalReady: false });
var r = await Promise.race([pending, new Promise((res) => setTimeout(() => res("TIMEOUT"), 1500))]);
var v = r.value;
console.log(`armed-first read after ${Date.now() - t0}ms:`, r === "TIMEOUT" ? "TIMEOUT" : [...v || []].map((x) => x.toString(16).padStart(2, "0")).join(" "));
await port.close();
