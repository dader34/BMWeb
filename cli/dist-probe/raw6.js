...
// ../../probe/raw6.ts
var fire = null;
var fake = { write: async () => {
}, onData: (fn) => {
  fire = fn;
}, set: async () => {
}, get: async () => null, close: async () => {
} };
var port = new NodeSerialPort("/dev/fake", async () => fake);
await port.open({ baudRate: 9600, dataBits: 8, stopBits: 1, parity: "even" });
var reader = port.readable.getReader();
var orphan = reader.read();
var first = await Promise.race([orphan, new Promise((res) => setTimeout(() => res("TIMEOUT"), 50))]);
console.log("read 1:", first === "TIMEOUT" ? "TIMEOUT (abandoned)" : "got data");
fire(new Uint8Array([170, 187]));
var second = await Promise.race([reader.read(), new Promise((res) => setTimeout(() => res("TIMEOUT"), 200))]);
console.log("read 2:", second === "TIMEOUT" ? "TIMEOUT <- the bytes went to the orphaned read" : "got " + [...second.value].map((x) => x.toString(16)).join(" "));
console.log("orphan holds:", [...(await orphan).value || []].map((x) => x.toString(16)).join(" "));
process.exit(0);
