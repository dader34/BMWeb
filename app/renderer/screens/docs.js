...
const DOCS_TYPE_KIND = {
  // reuse the wiring dot colours by kind
  REP: 'test',
  FEB: 'test',
  REH: 'test',
  PIB: 'pins',
...
async function docsHasChassis(chassis) {
  ...
  if (typeof BMACW_DOCS === 'object' && BMACW_DOCS && BMACW_DOCS[id])
    return true;
...
async function loadDocs(chassisId) {
  ...
  if (inline) {
    ...
    if (!r.ok)
      throw new Error(`no reference documents shipped for ${dispChassis(id)}`);
