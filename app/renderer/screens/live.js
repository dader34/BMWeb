...
function attachInpaPager(panel, grid, orderedKeys, cellFor) {
  ...
  function perPage() {
    ...
    const avail = grid.getBoundingClientRect
      ? window.innerHeight - grid.getBoundingClientRect().top - footer
      : 0;
  ...
  function relayout() {
    ...
    arrow.setAttribute('aria-label', atBottom ? 'Previous page' : 'Next page');
  ...
  const onKey = (e) => {
    ...
    if (n <= 1) return;
    const next =
      e.key === 'ArrowDown' ? Math.min(n - 1, page + 1) : Math.max(0, page - 1);
    if (next === page) return;
    e.preventDefault();
    page = next;
    relayout();
...
async function showInpaScreens(
  ecu,
  screens,
  container,
  title,
  ...
  const vmPopErrors = (scr, liveStatus) => {
    ...
    const sub = (t) =>
      String(t == null ? '' : t).replace(
        /ERROR_ECU_NICHT_VORHANDEN/g,
        liveStatus || 'no answer'
      );
    (async () => {
      for (const m of scr.vmErrors) {
        await messageDialog({
          title: esc(sub(m.title)) || 'INPA',
          body: esc(sub(m.body)),
        });
      }
    })();
  ...
  async function tick() {
    ...
    for (const scr of screens) {
      ...
      for (const r of gridOrder(scr)) {
        ...
        const unit =
          r.kind === 'lamp' ? null : r.unit || irUnitFor(r.key, vals);
    ...
    if (alive) {
      ...
      container.innerHTML = errorBlock(lastErr && lastErr.message);
      sbLeft.textContent = 'failed';
    } else {
...
const BOOL_ON =
  /^(ein|on|aktiv|active|ja|yes|1|betaetigt|gedrueckt|vorhanden)$/i;
const BOOL_OFF =
  /^(aus|off|nicht aktiv|not active|inaktiv|nein|no|0|nicht betaetigt|nicht vorhanden|not present)$/i;
...
function updateGaugeSpec(cellEl, rowSpec, raw) {
  ...
  if (rowSpec.okMin != null && rowSpec.okMax != null) {
    ...
    const a = at(rowSpec.okMin),
      b = at(rowSpec.okMax);
    const g = 'var(--gauge-ok)',
      r = 'var(--gauge-warn)';
...
function scheduleLive(tick) {
  ...
  const loop = async () => {
    ...
    try {
      ...
      // ticks render their own errors (and may call stopLive)
      /* an unexpected throw shouldn't kill the loop */
    }
...
function updateGauge(cellEl, key, raw) {
  ...
  const numStr = Number.isInteger(p.num)
    ? String(p.num)
    : Math.abs(p.num) >= 100
      ? p.num.toFixed(1)
      : p.num.toFixed(2);
...
async function watchMulti(ecu, jobs, container, view) {
  ...
  const stop = () => {
    ...
    container.querySelector('.live-title').textContent =
      `Stopped · ${jobs.length} job${jobs.length === 1 ? '' : 's'}`;
  ...
  function toggleLogging() {
    if (!window.bmacw) {
      sbLeft.textContent = 'logging unavailable';
      return;
    }
    if (logId) {
      stopLogging();
      logBtn.textContent = 'Stream to file…';
      meta.classList.remove('rec');
      sbLeft.textContent = 'log saved';
      return;
    }
    logColumns = [...cellEls.keys()];
    if (logColumns.length === 0) {
      sbLeft.textContent = 'no values yet';
      return;
    }
    ...
    window.bmacw.startLog(name, header).then((res) => {
      if (res && res.ok) {
        ...
        meta.classList.add('rec');
        sbLeft.textContent = `logging → ${res.path.split('/').pop()}`;
      }
    });
  }
  ...
  async function readAll() {
    ...
    await Promise.all(
      jobs.map(async (job) => {
        try {
          const data = await api(`/api/ecu/${ecu.sgbd}/run/${job}`, {
            method: 'POST',
          });
          flatResults(data.sets).forEach(([k, v]) => merged.set(k, v));
        } catch {
          /* one job failing shouldn't kill the whole watch */
        }
      })
    );
    return merged;
  ...
  async function tick() {
    ...
    if (logId && logColumns) {
      window.bmacw.appendLog(logId, [
        new Date().toISOString(),
        String(Date.now() - logStart),
