...
(function () {
  ...
  async function loadIndex() {
    ...
    loading = (async () => {
      ...
      try {
        ...
        if (!r || !r.ok) { INDEX = { sp: {} }; return INDEX; }
        const buf = new Uint8Array(await r.arrayBuffer());
        let text;
        const isGz = buf.length > 2 && buf[0] === 0x1f && buf[1] === 0x8b;
        if (isGz) {
          if (typeof fflate === 'undefined') { INDEX = { sp: {} }; return INDEX; }
  ...
  const BODY_TO_ISTA = {
    LIM: 'LIM', COU: 'COU', CAB: 'CAB', ROA: 'ROA', SAV: 'SAV', SAC: 'SAC',
    SAT: 'SAT', MPV: 'MPV', TOU: 'TOU', COM: 'COM', CM: 'COM', HB: 'HAT',
    HC: 'HAT', GC: 'HAT', GT: 'HAT',
  };
