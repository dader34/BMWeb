...
const DE_TOKENS = [
  ...
  [/\bPruefen\b|\bPrüfen\b/gi, 'Check'],
  [/\bLesen\b/gi, 'Read'],
  [/\bSchreiben\b/gi, 'Write'],
  [/\bSetzen\b/gi, 'Set'],
  [/\bLoeschen\b|\bLöschen\b/gi, 'Clear'],
  [/\bSteuern\b/gi, 'Activate'],
  [/\bSignatur\b/gi, 'Signature'],
  [/\bBlocklaenge\b|\bBlocklänge\b/gi, 'Block length'],
  [/\bZeiten\b/gi, 'Times'],
  ...
  [
    /Gibt das aktuelle gew(ä|ae)hlte Protokoll aus/gi,
  ...
  [
    /Alle m(ö|oe)glichen Diagnose-?Protokolle/gi,
    'all available diagnostic protocols',
  ],
  [
  ...
  [
    /aus dem Steuerger(ä|ae)t ausgelesene Daten im Format/gi,
    'data read from the ECU, formatted',
  ],
  ...
  [
    /Readyness Flag \(Standard-?Fehlerart\) als (Zahl|Text)/gi,
    'readiness flag',
  ],
  [
    /Fehler vorhanden \(Standard-?Fehlerart\) als (Zahl|Text)/gi,
    'fault present',
  ],
  ...
  [
    /Anzahl der CBS ?-? ?Umfaenge im Steuerger(ä|ae)t/gi,
    'number of CBS items in the ECU',
  ],
  ...
  [
    /Offset f(ü|ue)r letztes Anwender-?Infofeld/gi,
    'offset of the last user info field',
  ],
  [/Anwender-?Infofelder/gi, 'user info fields'],
  [/Anwender-?Infofeld/gi, 'user info field'],
  [/Endekennung/gi, 'end marker'],
  [/Maxanzahl/gi, 'max count'],
  [/\bAnzahl\b/gi, 'count'],
  [/\bAdresse\b/gi, 'address'],
  [/Gr(ö|oe)(ß|ss)e/gi, 'size'],
  [/\bletztes?\b/gi, 'last'],
  ...
  [/Heckklappe/gi, 'boot lid'],
  [/Heckscheibe/gi, 'rear window'],
  ...
  [/Entriegeln/gi, 'unlock'],
  [/Verriegeln/gi, 'lock'],
  [/\bTaster\b/gi, 'button'],
  [/\bSchalter\b/gi, 'switch'],
  [/\bKontakt\b/gi, 'contact'],
  [/\bRelais\b/gi, 'relay'],
  [/\bVersorgung\b/gi, 'supply'],
  [/\bStufe\b/gi, 'stage'],
  [/\bSender\b/gi, 'transmitter'],
  [/\bSchluessel\b|\bSchlüssel\b/gi, 'key'],
  [/\bReserve\b/gi, 'spare'],
  [/\bEingang\b/gi, 'input'],
  ...
  [/Datensatznummer/gi, 'dataset number'],
  [/Softwarenummer/gi, 'software number'],
  ...
  [/Einschaltzeit/gi, 'on-time'],
  [/Periodendauer/gi, 'period'],
  [/Tastverhältnis|Tastverhaeltnis/gi, 'duty cycle'],
  [/Abgleichs?wert/gi, 'adjustment value'],
  [/rueckwaerts|rückwärts/gi, 'backwards'],
  // fleet-wide ARG names + comments, by frequency. compounds precede fragments.
  [/Zusatzfunktion/gi, 'additional function'],
  [/Funktionale?r?/gi, 'functional'],
  [/Funktionen/gi, 'functions'],
  [/Funktion/gi, 'function'],
  [/Abgleichmenge/gi, 'adjustment quantity'],
  [/Abgleichflag/gi, 'adjustment flag'],
  [/Vorgabewert/gi, 'default value'],
  [/Vorgabe[Bb]yte/gi, 'default byte'],
  [/Vorgabespeed/gi, 'default speed'],
  [/Vorgabe/gi, 'default'],
  [/Querbeschleunigung/gi, 'lateral acceleration'],
  [/Drehzahl/gi, 'RPM'],
  [/Sollspannung/gi, 'target voltage'],
  [/Sensespannung/gi, 'sense voltage'],
  [/Sensorversorgung/gi, 'sensor supply'],
  [/Programmierspannung/gi, 'programming voltage'],
  // compound *spannung before the generic Spannung->voltage, else "...voltage"
  [/Batteriespannung/gi, 'battery voltage'],
  [/Versorgungsspannung/gi, 'supply voltage'],
  [/Unterspannung/gi, 'undervoltage'],
  ...
  [/Verstellwinkel/gi, 'adjustment angle'],
  [/Zuendwinkel|Zündwinkel/gi, 'ignition angle'],
  [/Drosselklappenwinkel/gi, 'throttle angle'],
  [/Motorlagewinkel/gi, 'engine position angle'],
  [/\bWinkel\b/gi, 'angle'],
  [/WortAdresse/gi, 'word address'],
  [/Speicheradresse/gi, 'memory address'],
  [/Diagnoseadresse/gi, 'diagnostic address'],
  [/Deviceadresse/gi, 'device address'],
  [/Codierblock/gi, 'coding block'],
  [/Steuerwert/gi, 'control value'],
  [/Steuerart\s*(\d+)/gi, 'control type $1'],
  [/Steuerparameter/gi, 'control parameter'],
  [/Ventilstellung/gi, 'valve position'],
  [/Fahrzeugh(ö|oe)he/gi, 'vehicle height'],
  [/Restlaufleistung/gi, 'remaining mileage'],
  [/Tankentl(ü|ue)ftung/gi, 'tank ventilation'],
  [/Abschaltung/gi, 'shutoff'],
  [/Aktivierung/gi, 'activation'],
  [/Betaetigung|Betätigung/gi, 'actuation'],
  [/bet(ä|ae)tigt/gi, 'actuated'],
  [/Kalibrieranforderung/gi, 'calibration request'],
  [/(ü|ue)berwachenden?/gi, 'monitored'],
  [/Verd(ä|ae)chtigung/gi, 'suspicion'],
  [/Pruefdatum|Pr(ü|ue)fdatum/gi, 'test date'],
  [/Inspektion/gi, 'inspection'],
  [/Produktion/gi, 'production'],
  [/Berechnung/gi, 'calculation'],
  [/Taktverh(ä|ae)ltnis/gi, 'duty cycle'],
  [/Schaltmodus/gi, 'switch mode'],
  [/Schalter/gi, 'switch'],
  [/\bAktion\b/gi, 'action'],
  [/Reaktion/gi, 'reaction'],
  [/Zaehler|Zähler/gi, 'counter'],
  [/Einheit/gi, 'unit'],
  [/Stellung/gi, 'position'],
  [/Helligkeit/gi, 'brightness'],
  [/Lautst(ä|ae)rke/gi, 'volume'],
  [/Leitung/gi, 'circuit'],
  [/Versorgung/gi, 'supply'],
  [/K(ü|ue)hler/gi, 'cooler'],
  [/Messung/gi, 'measurement'],
  [/Ergebnis/gi, 'result'],
  [/Beschreibung/gi, 'description'],
  [/einschl(ä|ae)ft/gi, 'sleeps'],
  [/m(ö|oe)gliche/gi, 'possible'],
  [/optional/gi, 'optional'],
  [/Zahl\b/gi, 'number'],
  // -index compound before the bare Aenderung rule, else "changesindex"
  [/(Ä|Ae|ä|ae)nderungsindex/gi, 'change index'],
  [/(\d+)-?stellig/gi, '$1-digit'],
  [/\bstellig/gi, 'digit'],
  [/\bZiffern?\b/gi, 'digits'],
  [/\binkl\.?/gi, 'incl.'],
  [/\bexkl\.?/gi, 'excl.'],
  [/\bAei\b/gi, 'AEI'],
  [/\bAe\b/gi, 'AE'], // INPA arg-code fragments, keep as-is
  // BMW field-code abbreviations (Fg=Fahrgestell, Zb=Zusammenbau, Sw=Software, Ds=Datensatz)
  [/\bDatum\b/gi, 'date'],
  [/\bFg\s*Nr\b/gi, 'chassis no.'],
  [/\bZb\s*Nr\b/gi, 'assembly no.'],
  [/\bSw\s*Nr\b/gi, 'software no.'],
  [/\bDs\s*Nr\b/gi, 'dataset no.'],
  [/\bHw\s*Nr\b/gi, 'hardware no.'],
  [/\bNr\b/gi, 'no.'],
  [/Signaturtestzeit/gi, 'signature test time'],
  [/\bSignatur\b/gi, 'signature'],
  [/\bBereich\b/gi, 'area'],
  [/\bProgramm\b/gi, 'Program'],
  [/vorgefuellter|vorgefüllter/gi, 'pre-filled'],
  [/Binaer\s?buffer|Binärbuffer/gi, 'binary buffer'],
  [/\bBinaer\b|\bBinär\b/gi, 'binary'],
  [/\bAls\b/gi, 'as'],
  // xenon leveling (SPU): ARG names abbreviate hor/ver + Wink(el)/Plaus(ibilitaet)
  [/Dejustagewinkels?/gi, 'misalignment angle'],
  [/Dejuhor\b/gi, 'horizontal misalignment'],
  [/Dejuver\b/gi, 'vertical misalignment'],
  [/Plausibilit(ä|ae)t/gi, 'plausibility'],
  [/\bPlaus\b/gi, 'plausibility'],
  [/\bWink\b/gi, 'angle'],
  [/normierter?/gi, 'normalized'],
  [/horizontaler?/gi, 'horizontal'],
  [/vertikaler?/gi, 'vertical'],
  [/Pruefstempel|Pr(ü|ue)fstempel|Pruefstemp\b/gi, 'inspection stamp'],
  [/Pruefcode|Pr(ü|ue)fcode/gi, 'test code'],
  [/Pruefflag|Pr(ü|ue)fflag/gi, 'test flag'],
  [/Auswahlbyte/gi, 'selection byte'],
  // numbered value/byte suffixes before the bare forms
  [/\bWert\s*(\d+)/gi, 'value $1'],
  [/\bByte\s*(\d+)/gi, 'byte $1'],
  [/\bWert\b/gi, 'value'],
  [/\bByte\b/gi, 'byte'],
  [/Injektor-?Mengenabgleich/gi, 'injector quantity adjustment (IMA)'],
  [/\bIma\b/gi, 'IMA'],
  [/Verstellwert/gi, 'adjustment value'],
  [/Verstellung/gi, 'adjustment'],
  [/Verstellen/gi, 'adjust'],
  [/\bAbgleich\b/gi, 'adjustment'],
  [/Programmieren/gi, 'programming'],
  [/\bZyl(?:inder)?\s*(\d+)/gi, 'cylinder $1'],
  [/\bZyl(?:inder)?\b/gi, 'cylinder'],
  [/Kennfeld/gi, 'map'],
  [/Ansteuerung/gi, 'control'],
  // fleet-wide job-argument vocabulary. compounds precede fragments.
  [/Codierdaten/gi, 'coding data'],
  [/Codierwert/gi, 'coding value'],
  [/Programmdaten/gi, 'program data'],
  [/Herstellerdaten/gi, 'manufacturer data'],
  [/Abgleichdaten/gi, 'adjustment data'],
  [/Ident[_ ]?Daten/gi, 'ident data'],
  [/Startadresse/gi, 'start address'],
  [/Startwert/gi, 'start value'],
  [/Offsetwert/gi, 'offset value'],
  [/Grenzwert/gi, 'limit value'],
  [/Adaptionswert/gi, 'adaptation value'],
  [/Analogwert/gi, 'analog value'],
  [/Digitalwert/gi, 'digital value'],
  [/Dezimalwert/gi, 'decimal value'],
  [/Hexwert/gi, 'hex value'],
  [/Dimmwert/gi, 'dim value'],
  [/Dummy[Ww]ert/gi, 'dummy value'],
  [/Abgleichspannung/gi, 'adjustment voltage'],
  [/Lambdasondenheizung/gi, 'lambda sensor heater'],
  [/Drehzahlanhebung/gi, 'idle speed increase'],
  [/Solldrehzahl/gi, 'target RPM'],
  // Soll- compound before the bare noun
  [/Soll-?F(ö|oe)rdermenge/gi, 'target delivery quantity'],
  [/F(ö|oe)rdermenge/gi, 'delivery quantity'],
  [/F(ö|oe)rderbeginn/gi, 'delivery start'],
  [/Enddrehzahl/gi, 'end RPM'],
  [/Drehrichtung/gi, 'rotation direction'],
  [/Bewegungsrichtung/gi, 'movement direction'],
  [/Prozentschritten/gi, 'percent steps'],
  [/Schrittanzahl/gi, 'number of steps'],
  [/Schrittmotoren/gi, 'stepper motors'],
  [/Kompressorkupplung/gi, 'compressor clutch'],
  [/Eigenraderkennung/gi, 'own-wheel detection'],
  [/Bitkombinationen/gi, 'bit combinations'],
  [/Innenbeleuchtung/gi, 'interior lighting'],
  [/Spiegelheizung/gi, 'mirror heater'],
  [/Luftverteilung/gi, 'air distribution'],
  [/Luftfuehrung/gi, 'air guidance'],
  [/Motorlagersteuerung/gi, 'engine mount control'],
  [/Sendeleistung/gi, 'transmit power'],
  [/Behoerden(daten)?/gi, 'authority data'],
  [/Vindaten/gi, 'VIN data'],
  [/Klangzeichen/gi, 'chime'],
  [/Heimleuchten/gi, 'welcome light'],
  // verbs (infinitive + inflected)
  [/einschalten/gi, 'switch on'],
  [/ausschalten/gi, 'switch off'],
  [/aktivieren/gi, 'activate'],
  [/deaktivieren/gi, 'deactivate'],
  [/eingeben/gi, 'enter'],
  [/vorgeben/gi, 'specify'],
  [/vorzugebenden?/gi, 'to be specified'],
  [/uebergeben|übergeben/gi, 'pass'],
  [/uebernehmen|übernehmen/gi, 'apply'],
  [/auszulesenden?/gi, 'to be read'],
  [/lesenden?/gi, 'read'],
  [/gelesen/gi, 'read'],
  [/ausgelesen/gi, 'read out'],
  [/geschrieben/gi, 'written'],
  [/codiert/gi, 'coded'],
  [/angesteuert/gi, 'activated'],
  [/gewünschte|gewuenschte/gi, 'desired'],
  [/ausgewählten|ausgewaehlten/gi, 'selected'],
  [/unveraendert|unverändert/gi, 'unchanged'],
  [/dokumentierung/gi, 'documentation'],
  [/behält|behaelt/gi, 'keeps'],
  [/löscht|loescht/gi, 'clears'],
  [/steuert/gi, 'controls'],
  [/schalten/gi, 'switch'],
  [/starten/gi, 'start'],
  [/sperren/gi, 'lock'],
  ...
  [/\bDaten\b/gi, 'data'],
  [/Bezeichnung/gi, 'designation'],
  [/Quantisierung/gi, 'quantization'],
  [/Umrechnung/gi, 'conversion'],
  [/Belegung/gi, 'assignment'],
  [/Belueftung|Belüftung/gi, 'ventilation'],
  [/Entfrostung/gi, 'defrost'],
  [/Beleuchtung/gi, 'lighting'],
  [/Heizung/gi, 'heater'],
  [/Kalibrierung/gi, 'calibration'],
  [/Codierung/gi, 'coding'],
  [/Steuerung/gi, 'control'],
  [/Regelung/gi, 'regulation'],
  [/Deaktivierung/gi, 'deactivation'],
  [/Initialisierung/gi, 'initialization'],
  [/Verfuegbarkeit|Verfügbarkeit/gi, 'availability'],
  [/Wiederholung/gi, 'repetition'],
  [/Einstellung/gi, 'setting'],
  [/Gewichtung/gi, 'weighting'],
  [/Verbindung/gi, 'connection'],
  [/Einspritzung/gi, 'injection'],
  [/Beladung/gi, 'load'],
  [/Dichtung/gi, 'seal'],
  [/Kennlinien?/gi, 'characteristic curve'],
  [/Serien\b/gi, 'series'],
  [/Grenz\b/gi, 'limit'],
  [/Steigung/gi, 'slope'],
  [/Spreizung/gi, 'spread'],
  [/Abweichung/gi, 'deviation'],
  [/Aenderung|Änderung/gi, 'change'],
  ...
  [/\bRohsignale\b/gi, 'raw signals'],
  [/\bRohsignal\b/gi, 'raw signal'],
  ...
  [/Tasten/gi, 'buttons'],
  [/Lampen/gi, 'lamps'],
  [/Antennen/gi, 'antennas'],
  [/Sekunden/gi, 'seconds'],
  [/Schichtung/gi, 'stratification'],
  [/Richtung/gi, 'direction'],
  [/Kennung/gi, 'ID'],
  [/Länge|Laenge/gi, 'length'],
  [/Gruen\b/gi, 'green'],
  [/Abblenden/gi, 'dim'],
  [/\bUnten\b/gi, 'down'],
  [/\bAussen\b/gi, 'outside'],
  [/muessen|müssen/gi, 'must'],
  [/sollen/gi, 'should'],
  [/\bwerden\b/gi, 'are'],
  [/\balten\b/gi, 'old'],
  [/\bfolgenden\b/gi, 'following'],
  [/\bzwischen\b/gi, 'between'],
  [/\büber\b|\buebe?r\b/gi, 'via'],
  [/ACHTUNG/gi, 'ATTENTION'],
  [/\bVvten\b/gi, 'VANOS'],
  [/freibrennen|freebrennen/gi, 'burn-off'],
  ...
  [/ohne Argument/gi, 'without argument'],
  [/Wechsel/gi, 'toggle'],
  [/Klima und Fahrbedingung/gi, 'A/C and driving condition'],
  [/mit Klimaanlage/gi, 'with A/C'],
  [/mit Fahrstufe/gi, 'with gear engaged'],
  ...
  [/\bnicht aktiv\b/gi, 'not active'],
  [/\bnicht bereit\b/gi, 'not ready'],
  [/\bnicht vorhanden\b/gi, 'not present'],
  [/\bnicht erkannt\b/gi, 'not detected'],
  [/\baktiv\b/gi, 'active'],
  [/\bbereit\b/gi, 'ready'],
  [/\bgesperrt\b/gi, 'locked'],
  [/\bja\b/gi, 'yes'],
  [/\bnein\b/gi, 'no'],
  [/\bfehlerfrei\b/gi, 'no fault'],
  [/\bEin\b/gi, 'on'],
  [/\bAus\b/gi, 'off'],
  [/\bZeit\b/gi, 'time'],
  [/\bDauer\b/gi, 'duration'],
  [/\bFaktor\b/gi, 'factor'],
  [/\bbis\b/gi, 'to'],
  ...
  // component nouns
  [/Drehzahlfühler/gi, 'speed sensor'],
  [/Drehzahlsensor/gi, 'speed sensor'],
  [/Lenkwinkel ?[Ss]ensor/gi, 'steering angle sensor'],
  [/Lenkwinkel/gi, 'steering angle'],
  [/Drucksensor/gi, 'pressure sensor'],
  [/Druck ?[Ss]ensor/gi, 'pressure sensor'],
  ...
  [/Lichtmodul/gi, 'light module'],
  [/Lichtmaschine/gi, 'alternator'],
  [/sporadischer Fehler/gi, 'intermittent fault'],
  [
    /ungültiger Arbeitsbereich|ungueltiger Arbeitsbereich/gi,
    'invalid operating range',
  ],
  [/keine CAN ID/gi, 'no CAN ID'],
  [/CAN ID/gi, 'CAN ID'],
  [/momentan vorhanden/gi, 'currently present'],
  [/nicht vorhanden/gi, 'not present'],
  ...
  [/Z(ü|ue)ndkreis/gi, 'squib circuit'],
  [/Gurtstrammer|Gurtstraffer/gi, 'belt tensioner'],
  [/Seitenairbag/gi, 'side airbag'],
  [/Kopfairbag/gi, 'head airbag'],
  [/Beifahrerairbag/gi, 'passenger airbag'],
  [/Fahrerairbag/gi, 'driver airbag'],
  [/\bStufe\b/gi, 'stage'],
  [/Crashsensor/gi, 'crash sensor'],
  [/Sitzbelegungserkennung/gi, 'seat occupancy detection'],
  [/Fehlerlampe/gi, 'warning lamp'],
  [/\bAirbag\b/gi, 'airbag'],
  // supply / communication
  [/Kommunikation/gi, 'communication'],
  [/Masse-?Schluss/gi, 'short to ground'],
  [/Widerstand zu gro(ß|ss)/gi, 'resistance too high'],
  [/Widerstand zu klein/gi, 'resistance too low'],
  [/Fensterheber/gi, 'window lift'],
  [/Zentralverriegelung/gi, 'central locking'],
  [/Beifahrerspiegel/gi, 'passenger mirror'],
  [/Fahrerspiegel/gi, 'driver mirror'],
  [/Beifahrerseite/gi, 'passenger side'],
  [/Fahrerseite/gi, 'driver side'],
  [/\bBeifahrer\b/gi, 'passenger'],
  [/\bFahrer\b/gi, 'driver'],
  [/Potentiometer/gi, 'potentiometer'],
  [/Achse/gi, 'axis'],
  [/Sicherung/gi, 'fuse'],
  [/Relais/gi, 'relay'],
  [/Motor/gi, 'motor'],
  [/Schlüssel|Schluessel/gi, 'key'],
  [/Toleranz/gi, 'tolerance'],
  [/erhöht|erhoeht/gi, 'increased'],
  [/Impulsrad/gi, 'reluctor ring'],
  [/Überwachung|Ueberwachung/gi, 'monitoring'],
  [/\bNummer\b/gi, 'number'],
  [/\bbei\b/gi, 'at'],
  [/\boder\b/gi, 'or'],
  [/Luftsystem/gi, 'air system'],
  [/Luftmasse/gi, 'air mass'],
  [/Kraftstoffsystem/gi, 'fuel system'],
  [/Zündsystem/gi, 'ignition system'],
  [/Generator/gi, 'alternator'],
  [/Lichtmaschine/gi, 'alternator'],
  [/Botschaft/gi, 'message'],
  [/Antwort/gi, 'response'],
  // generic tokens
  [/Übertemperatur/gi, 'over-temperature'],
  [/Untertemperatur/gi, 'under-temperature'],
  [/Leitungsunterbrechung/gi, 'open circuit'],
  [/Unterbrechung/gi, 'open circuit'],
  [/Kurzschluss/gi, 'short circuit'],
  [/unterhalb Schwelle/gi, 'below threshold'],
  [/oberhalb Schwelle/gi, 'above threshold'],
  [/hinten rechts/gi, 'rear right'],
  [/hinten links/gi, 'rear left'],
  [/vorne rechts/gi, 'front right'],
  [/vorne links/gi, 'front left'],
  [/rechts/gi, 'right'],
  [/links/gi, 'left'],
  [/hinten/gi, 'rear'],
  [/vorne/gi, 'front'],
  [/periodische/gi, 'periodic'],
  [/implausible/gi, 'implausible'],
  [/falsch/gi, 'wrong'],
  [/keine/gi, 'no'],
  [/gegen Masse/gi, 'to ground'],
  [/Masse/gi, 'ground'],
  [/unplausibel/gi, 'implausible'],
  [/erkannt/gi, 'detected'],
  [/Signal/gi, 'signal'],
  [/Fehler/gi, 'fault'],
  [/frei/gi, 'free'],
...
function deGerman(text) {
  ...
  if (out === null)
    for (const [de, en] of FAULT_PHRASES)
      if (trimmed === de) {
        out = en;
        break;
      }
