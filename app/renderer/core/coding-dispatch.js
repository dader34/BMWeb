...
(function (root) {
  ...
  class CdhHost {
    ...
    CDHGetApiJobData(maxData, bufHandle) {
      ...
      binWriteByte(buf, 19, 0);
      binWriteByte(buf, 20, 0);
      // payload at 0x15
      for (let k = 0; k < n; k++) {
    ...
    CDHGetNettoDataFromCbd() {
      return 0;
    } // slots are supplied by the caller
  ...
  const api = {
    ...
    CdhHost,
    Interp,
    _makeBinBuf: makeBinBuf,
    _bytesToArgString: bytesToArgString,
    _binWriteWord: binWriteWord,
    _binReadWord: binReadWord,
  };
