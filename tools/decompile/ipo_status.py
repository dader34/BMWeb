...
def extract(path):
    """The Read-status entry an .IPO declares: a menu with its pages, or one screen.

    Args:
        path: The .IPO file.

    Returns:
        ``{"ecu": stem, "status": {"kind": "menu"|"screen", ...} | None}``.
    """
...
def main():
    ...
    ns = parse_args(__doc__,
                    ...
                    flags={"--write": "write the corpus result to _status.json"})
