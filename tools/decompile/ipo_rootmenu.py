...
def extract(path):
    """The root menu an .IPO declares, with INPA's own F-key numbering.

    Args:
        path: The .IPO file.

    Returns:
        ``{"ecu": stem, "items": [...], "deadKeys": [...]?}``; `items` is
        empty when no root menu decodes.
    """
...
def main():
    ...
    ns = parse_args(__doc__,
                    ...
                    flags={"--write": "write the corpus result to _rootmenus.json"})
