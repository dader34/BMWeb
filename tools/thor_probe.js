...
async function ident(cmd, respLen, timeoutMs = 3000) {
  ...
  if (rx.length < want) throw new Error(`0x${cmd.toString(16)}: ${rx.length}/${want} bytes (${hex(rx) || 'nothing'})`);
...
(async () => {
  ...
  console.log(`ignition:  raw ${hex(ign)}  -> status ${ign[4]}`
    + ` (${ign[4] & 0x01 ? 'ON' : 'off'})`);

  const fw = await ident(0xFD, 9);
  const type = (fw[4] << 8) | fw[5];
  const ver = (fw[6] << 8) | fw[7];
  console.log(`firmware:  raw ${hex(fw)}  -> adapter type ${type}, version ${ver}`);

  if (type >= 2) {
    const ser = await ident(0xFB, 13);
    console.log(`serial:    ${hex(ser.slice(4, 12))}`);
    const volt = await ident(0xFC, 6);
    console.log(`battery:   raw value ${volt[4]}  (~${(volt[4] / 10).toFixed(1)} V if /10)`);
  } else {
  ...
  sock.destroy();
})().catch((e) => { console.error('probe failed:', e.message); sock.destroy(); process.exit(1); });
