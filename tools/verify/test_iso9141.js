...
{
  ok(
    !/this transport does not implement/.test(src),
    'the "not implemented" refusal is gone'
  );
  ok(
    /const isIso9141 = \(c\) => c === 0x10c;/.test(src),
    'isIso9141 identifies concept 0x10C'
  );
  ok(
    /ISO9141_INIT_ADDR = 0x33/.test(src),
    'the generic tester address 0x33 is used, not the ECU KWP address'
  );
  ok(/ISO9141_BAUD = 10400/.test(src), 'ISO 9141 runs at 10400 baud, not 9600');
}
...
console.log('\nport settings');
{
  ...
  ok(
    kwp.baudRate === 9600 && kwp.parity === 'even',
...
console.log('\nframing, against the real answer from the car');
{
  ...
  ok(
    total === 12,
    `frame length from the header: 8 data + 3 hdr + 1 sum = 12 (got ${total})`
  );
  ok(
    sandbox.frameTotal([], { concept: 0x10c }) === null,
    'undecidable with no bytes yet'
  );
  // and a checksum still computes as sum8, shared with KWP
  const framed = sandbox.withChecksum([0x68, 0x6a, 0xf1, 0x01, 0x00], {
    concept: 0x10c,
  });
  const want = (0x68 + 0x6a + 0xf1 + 0x01 + 0x00) & 0xff;
  ok(
    framed[framed.length - 1] === want,
...
console.log('\nsession lifecycle');
{
  ...
  ok(
    /if \(framed && !this\.inited && wantsWake\)/.test(src),
  ...
  ok(
    /bus\.sessionConcept = conceptOf\(comm\);/.test(src),
    'the session tracks the current concept, and every xsetpar updates it'
  );
  ok(
    /await bus\.ensureConfig\(portConfig\(comm\)\);/.test(src),
...
console.log('\nBMW K-line fast init (the wake that was missing)');
{
  ...
  ok(
    /dataTerminalReady: true, break: true/.test(fi),
  ...
  ok(
    /const isKline = \(c\) => isDs2\(c\) \|\| c === 0x10d;/.test(src),
...
console.log('\nthe wake lives on the bus that owns the wires');
{
  ...
  ok(
    /const wantsWake = isIso9141\(concept\) \|\| isIso9141\(this\.sessionConcept\)/.test(
      web
    ),
    'only ISO 9141 asks for a wake'
  );
  ok(
