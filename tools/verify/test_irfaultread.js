...
const faultRead = src.match(/const IR_FAULT_READ = new RegExp\([\s\S]*?\);\n/)[0];
const faultJob = src.match(/const IR_FAULT_JOB = \[[\s\S]*?\];\n/)[0];
const env = new Function(
  memWord + faultRead + faultJob +
  'return { IR_FAULT_READ, IR_FAULT_JOB };')();
...
console.log('\nand it SURVIVES the empty-submenu filter');
{
  ...
  // memory".
  const filt = src.match(/\.filter\(it => !it\.menu \|\| it\.screen[\s\S]*?\);/);
  ok(filt !== null, 'the empty-submenu filter is still there');
  ok(filt && /IR_FAULT_READ\.test\(it\.label\)/.test(filt[0]),
...
console.log('\nagainst the real IR corpus');
{
  ...
  let dead = 0, wouldRun = 0, clears = 0;
  for (const f of fs.readdirSync(dir).filter((x) => x.endsWith('.json'))) {
    ...
    for (const m of Object.values(menus)) {
      for (const it of (m.items || [])) {
        ...
        if (/^(Clear|Loesch|Lösch)/i.test(lbl)) { clears++; continue; }
        if (!env.IR_FAULT_READ.test(lbl) || it.job) continue;
        const sub = menus[it.menu];
        const subReads = sub && (sub.items || []).some(
          (x) => /^(FS|IS|HS)_/i.test(String(x.job || '')));
        const faultScreen = it.screen && (d.screens || {})[it.screen];
        if (!faultScreen && !subReads) { dead++; wouldRun++; }
      }
    }
  }
