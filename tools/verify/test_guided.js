...
const { IpoVm, FeedHost, scanQuitBox } = require('../../app/renderer/core/ipovm.js');
...
// ---- 1. the full Pruefung lifecycle, scripted -------------------------------
{
  ...
  const waits = [];
  const states = [];
  let guardSeen = null, pressed = false;
  ...
  assert.ok(names.includes('DIAGNOSE_TESTBIT'),
  ...
  const onIdx = jobs.findIndex(([j, a]) => j === 'STEUERN_STANDHEIZUNG' && a === 'EIN');
  assert.ok(onIdx > names.indexOf('STARTZAEHLER_LESEN'), 'heater ON before the counter read');
  // ...and the TEARDOWN runs (unreachable before the segment-relative jump fix)
  assert.ok(jobs.some(([j, a]) => j === 'STEUERN_STANDHEIZUNG' && a === 'AUS'
      && jobs.indexOf(jobs.find(([jj, aa]) => jj === j && aa === a)) >= 0),
    'no heater AUS at all');
  assert.ok(jobs.filter(([j]) => j === 'STARTZAEHLER_LESEN').length >= 2,
    'the second (teardown) counter read never ran');
  assert.ok(jobs.some(([j, a]) => j === 'DIAGNOSE_TESTBIT' && a === 'AUS'),
    'DIAGNOSE_TESTBIT AUS (teardown) never ran');
  assert.ok(waits.includes(2000) && waits.includes(10000),
    `the 2000/10000ms waits were skipped (${waits})`);
  assert.ok(states.includes('%STOP_PHASE') && states.includes('%NACHBEREITUNG'),
    `teardown states unreachable (${states})`);
  assert.ok(pressed && guardSeen && guardSeen[0] === '%NACHBEREITUNG'
      && guardSeen[1].includes(33),
    `the Weiter guard (slot 33 at %NACHBEREITUNG) was not offered: ${JSON.stringify(guardSeen)}`);
  ok(`Pruefung: ${jobs.length} jobs, waits ${JSON.stringify([...new Set(waits)])}, `
     + `states ${[...new Set(states)].join(' ')}, Weiter pressed`);

  // the counter fed through FeedHost surfaced in the machine's own print
  assert.ok(texts.some((t) => /Startz\u00e4hler : 7|Startzähler : 7/.test(t)),
...
// ---- 2. offline parity: wireJobs off keeps the old shape --------------------
{
  ...
  assert.strictEqual(step.kind, 'yield',
...
// ---- 3. the quit-box scan on ZKE5 -------------------------------------------
{
  ...
  assert.strictEqual(qb.slot, 29, `quit flag should be slot 29, got ${qb.slot}`);
