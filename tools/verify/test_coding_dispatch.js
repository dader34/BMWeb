...
function buildExec() {
  ...
  const args = [
    path.join(ROOT, 'tools/export/ipo_exec.py'),
    'A_KMB46',
    out,
    '--coding',
  ];
...
async function main() {
  ...
  if (exec.dataOrg) {
    ok(
      exec.dataOrg.wortBreite === 2,
      'KMB dataOrg is word mode (wortBreite 2)'
    );
  ...
  const jn = exec.procs.cabimain.find(
    (t) => t.op === 'const' && t.v === 'JOBNAME'
  );
  ...
  const slots = [];
  for (let a = 0; a < 20; a++)
    slots.push({ addr: a, value: 0x40 + a, mask: 0xff, flags: 0 });
  ...
  async function runJob(sgbd, job, argText, o) {
    ...
    if (/LESEN/.test(job))
      return [{ JOB_STATUS: 'OKAY', CODIERDATEN: '00'.repeat(20) }];
  ...
  ok(
    jobs.some((j) => j.job === 'IDENT'),
    'issued IDENT (coding index read)'
  );
  ok(
    jobs.some((j) => j.job === 'C_S_LESEN' && j.binary),
    'issued C_S_LESEN with a binary request'
  );
  ok(
    jobs.some((j) => j.job === 'C_CHECKSUM'),
    'issued C_CHECKSUM'
  );
  ...
  if (packet) {
    ...
    ok(
      (packet[13] | (packet[14] << 8)) === 20 * wb,
      `byte count @0x0D = ${20 * wb}`
    );
  ...
  let refused = false;
  try {
    await CD.runCodingDispatch(exec, { sgbd: 'C_KMB46', slots, runJob });
  } catch (e) {
    refused = /confirmed/.test(e.message);
  }
  ...
  let refused2 = false;
  try {
    await CD.runCodingDispatch(
      { procs: {}, coding: false },
      { confirmed: true, runJob }
    );
  } catch (e) {
    refused2 = /coding-dispatcher/.test(e.message);
  }
