...
vm.runInContext(
  fs.readFileSync(path.join(R, 'app/renderer/data/envmap.js'), 'utf8'),
  sandbox
);
const MAP = sandbox.window.BMW_ENV_TEXT;
assert.ok(
  MAP && typeof MAP === 'object',
  'envmap.js must set window.BMW_ENV_TEXT'
);
...
for (const [de, en] of Object.entries(known)) {
  ...
  assert.strictEqual(
    envLabel(de),
    en,
    `envLabel(${JSON.stringify(de)}) -> ${en}`
  );
}
ok(
  `envLabel translates the known env labels and enum values (${Object.keys(known).length} checked)`
);

// leading/trailing whitespace on the incoming string is trimmed before lookup
assert.strictEqual(
  envLabel('  Batteriespannung  '),
  'Battery voltage',
...
assert.ok(
  !(unknown in MAP),
  'the unknown string must not be in the dictionary'
);
ok(
  'envLabel passes an unknown string through unchanged (no heuristic munging)'
);
