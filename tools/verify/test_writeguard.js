...
for (const [name, want] of [
  ...
  ['IDENT', false],
  ['FS_LESEN', false],
  ['CODIERUNG_LESEN', false],
  ['MESSWERTBLOCK_LESEN', false],
  ['FS_LOESCHEN', true],
  ['STEUERN_E_LUEFTER', true],
  ['ADAP_SELEKTIV_LOESCHEN', true],
  ['CODIERUNG_SCHREIBEN', true],
  ['ECU_CONFIG_RESET', true],
  ['STEUERGERAETE_RESET', true],
]) {
...
if (!code0) {
  check(
    'behavioural half ran (missing data/chassis/.../ms450ds0/job-code.json' +
      ' -- regenerate with tools/sgbd/ecu_tree.py)',
    false
  );
  ...
  const c = fix.cases.find(
    (x) => x.sgbd === 'ms450ds0' && x.job === 'FS_LOESCHEN'
  );
  if (!c) {
    check(
    ...
    const byReq = new Map(
      (c.telegrams || []).map((t) => [String(t.request), t.response])
    );
        ...
        Object.assign(
          {
            ...
            send: (r) => {
              sent++;
              return byReq.get(String(Array.from(r))) ?? lastResp;
            },
          },
          opts
        )
    ...
    let threw = false;
    try {
      mk({ allowWrites: false }).run('FS_LOESCHEN');
    } catch (e) {
      threw = true;
    }
    ...
    check(
      'write job runs by default now',
      sent > 0 && Object.assign({}, ...sets).JOB_STATUS === 'OKAY'
    );
