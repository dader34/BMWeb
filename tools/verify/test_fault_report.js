...
let CAPTURED = null;
global.printHeading = (t) => ({ html: `<h2 class="pr-h2">${global.esc(t)}</h2>` });
global.printHtml = (h) => ({ html: h || '' });
global.printDoc = (opts) => { CAPTURED = opts; return Promise.resolve(); };
global.loadFaultMeta = async () => {};
global.loadPcodes = async () => {};

const R = require(path.join(__dirname, '..', '..',
  'app', 'renderer', 'screens', 'fault-report.js'));
...
function firstFaultRowCells(tableHtml, faultRowClass) {
  const re = new RegExp(`<tr class="[^"]*${faultRowClass}[^"]*">([\\s\\S]*?)</tr>`);
...
const detailFaults = [
  { code: 'P0301', pcode: 'P0301', hex: '', name: 'Cyl 1 misfire',
    present: true, ftype: 'static', count: '3' },
  { code: 'P0420', pcode: 'P0420', hex: '', name: 'Catalyst below threshold',
    present: false, ftype: 'intermittent', count: '1' },
  { code: '5DC2', pcode: '', hex: '', name: 'No detail on this one',
    present: false, ftype: '', count: '' },   // still 5 columns; type/count '—'
];
// A compact module (dsc_mk60-like): state only, no type/count, no env.
const compactFaults = [
  { code: '5DC2', pcode: '', hex: '', name: 'Wheel speed sensor FL',
    present: true, ftype: '', count: '' },
  { code: '5DC3', pcode: '', hex: '', name: 'Wheel speed sensor FR',
    present: false, ftype: '', count: '' },
];
// A body module (mrs4-like): env with Begin/Endefehleruhr, no type/count.
const bodyFaults = [
  { code: '00A2', pcode: '', hex: '', name: 'Crash sensor',
    present: false, ftype: '', count: '' },
];
...
// ---- 1. faultColumns is always the same five ------------------------------
{
  ...
  for (const c of cols) {
    if (c.key === 'name') assert.strictEqual(c.width, '',
      'Description must stay elastic (no fixed width)');
    else assert.ok(c.width, `${c.key} needs a fixed <colgroup> width to align across blocks`);
  }
...
function checkNative(label, sgbd, codes) {
  ...
  assert.strictEqual(p.cols, 5, `${label}: <colgroup> must have 5 <col>, got ${p.cols}`);
  const cells = firstFaultRowCells(html, 'faultrow');
  assert.strictEqual(cells, p.ths,
...
}
{
  ...
  assert.ok(/<col style="width:/.test(compact),
...
{
  ENV = new Map([[detailFaults[0], [['Begin', '1000 km'], ['Endefehleruhr', '55 h'],
                                    ['Battery', '13.10 V'], ['RPM', '750 1/min']]]]);
  ...
  const items = html.match(/<span class="e-item">[\s\S]*?<\/span><\/span>/g) || [];
  assert.strictEqual(items.length, 4, `expected 4 paired env items, got ${items.length}`);
  for (const it of items) {
    assert.ok(/class="e-k"/.test(it) && /class="e-v"/.test(it),
      'an env item lost its label or value -- pair broken');
  }
  // the fault row and its env row both carry break-avoid classes
  assert.ok(/<tr class="faultrow/.test(html) && /<tr class="envrow"/.test(html),
...
async function checkWeb(label, faultyList) {
  ...
  assert.ok(tables.length === faultyList.length,
    `${label}: expected ${faultyList.length} tables, got ${tables.length}`);
  for (const t of tables) {
    const p = tableParts(t);
    assert.strictEqual(p.ths, 5, `${label}: web table needs 5 <th>, got ${p.ths}`);
    assert.strictEqual(p.cols, 5, `${label}: web table needs 5 <col>, got ${p.cols}`);
    const cells = firstFaultRowCells(t, 'pr-faultrow');
    assert.strictEqual(cells, p.ths,
      `${label}: web fault row ${cells} <td> vs ${p.ths} <th>`);
    assert.ok(/table-layout|pr-fault-table/.test(t) || /pr-fault-table/.test(t),
...
async function main() {
  ...
  ENV = new Map();
  await checkWeb('detail', [{ ecu: { label: 'DME', sgbd: 'ms450ds0' }, codes: detailFaults }]);
  await checkWeb('compact', [{ ecu: { label: 'DSC', sgbd: 'dsc_mk60' }, codes: compactFaults }]);
  ok('web printFaultReport: 5 th == 5 col == 5 td for detail AND compact');

  // ---- 5. web env pairs stay paired + break-avoid -------------------------
  ENV = new Map([[bodyFaults[0], [['Begin', '1000 km'], ['Endefehleruhr', '55 h']]]]);
  CAPTURED = null;
  await R.printFaultReport('E46',
    [{ ecu: { label: 'MRS', sgbd: 'mrs4' }, codes: bodyFaults }],
    { scanned: 1, skipped: 0 });
  const table = CAPTURED.sections.map((s) => (s && s.html) || '')
    .find((h) => /pr-envtr/.test(h));
  assert.ok(table, 'web env row (pr-envtr) missing');
  const rows = table.match(/<div class="pr-env-row">[\s\S]*?<\/div>/g) || [];
  assert.strictEqual(rows.length, 2, `expected 2 env pairs, got ${rows.length}`);
  for (const r of rows) {
    assert.ok(/pr-env-k/.test(r) && /pr-env-v/.test(r), 'web env pair broke apart');
  }
