...
// ---- LLERH: +10 twice, then +100, then -10 ---------------------------------
{
  ...
  assert.deepStrictEqual(
    item('m_system_llerh', 1),
    [
      ['STOP_SYSTEMCHECK_LLERH', ''],
      ['START_SYSTEMCHECK_LLERH', '10'],
    ],
    '+10 must stop, then start with the computed setpoint'
  );
  assert.deepStrictEqual(
    item('m_system_llerh', 1).pop(),
    ['START_SYSTEMCHECK_LLERH', '20'],
    'a second +10 must accumulate to 20'
  );
  assert.deepStrictEqual(
    item('m_system_llerh', 3).pop(),
    ['START_SYSTEMCHECK_LLERH', '120'],
    '+100 on top must send 120'
  );
  assert.deepStrictEqual(
    item('m_system_llerh', 2).pop(),
    ['START_SYSTEMCHECK_LLERH', '110'],
    '-10 must send 110'
  );
  ok(
    'LLERH: +10, +10, +100, -10 -> 10, 20, 120, 110 (clamped setpoint, one VM)'
  );
}
...
// ---- Select ("Ausw"): the inputint prompt feeds the absolute target --------
{
  ...
  assert.ok(
    /Request set value idle speed/.test(prompt.texts[0]),
  ...
  assert.deepStrictEqual(
    ...
    [
      ['STOP_SYSTEMCHECK_LLERH', ''],
      ['START_SYSTEMCHECK_LLERH', '850'],
    ],
    'the typed 850 must become the START argument'
  );
  ok(
    "Select: prompts INPA's inputint (0..1999), 850 -> START_SYSTEMCHECK_LLERH 850"
  );
}
