...
const src = fs.readFileSync(
  path.join(ROOT, 'core', 'offline-export.js'),
  'utf8'
);
...
const refs = [
  ...html.matchAll(/<(?:script src|link[^>]*href)="([^"]+\.(?:js|css|svg))"/g),
].map((m) => m[1]);
...
console.log(
  `  offline-shell  ${shell.length} entries mirror index.html (${refs.length} refs)`
);
