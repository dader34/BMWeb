...
const words = (raw) => {
  ...
  for (let i = 0; i + 1 < raw.length; i += 2)
    w.push(raw[i] | (raw[i + 1] << 8));
  return w;
};
const blobs = (file) => {
  const g = load(file);
  return g.ops
    .filter((o) => o[0] === 'xsetpar' && o[1][0][0] === 8)
    .map((o) => g.strings[o[1][0][1]])
...
assert.deepStrictEqual(
  [by[0x6].timeout, by[0x6].regen],
  [500, 25],
  'DS2: [5],[6]'
);
assert.deepStrictEqual(
  [by[0x10d].timeout, by[0x10d].regen, by[0x10d].timeoutNr78],
  [500, 25, 5000],
  'KWP2000*: [2],[3], Nr78 [7]'
);
assert.deepStrictEqual(
  [by[0x10f].timeout, by[0x10f].regen, by[0x10f].timeoutNr78],
  [800, 20, 5000],
  'BMW-FAST: [2],[3], Nr78 [6]'
);
ok(
  'ms450ds0: DS2 500/25, KWP2000* 500/25 (Nr78 5000), BMW-FAST 800/20 (Nr78 5000)'
);

const d = blobs('data/groups/d_0012.json.gz');
assert.ok(
  d.every((c) => c.concept === 0x6 && c.timeout === 1000),
  'D_0012 probes DS2 at 1000'
);
assert.deepStrictEqual(
  d.map((c) => c.regen),
  [200, 100]
