...
(async () => {
  ...
  {
    const r = TM._parseRange(
      'Hexwert (0x00-0xFF) der WortAdresse ,ab der das EEPROM gelesen werden soll'
    );
    ok(r && r.lo === 0x00 && r.hi === 0xff, 'EEPROM address range 0x00-0xFF');
    const r2 = TM._parseRange(
      'Hexwert (0x0000-0xFFFF) der Adresse ,ab der das Rom gelesen werden soll'
    );
    ok(r2 && r2.hi === 0xffff, 'ROM address range 0x0000-0xFFFF');
    const r3 = TM._parseRange(
      'Hexwert (0x80-0xDF) der Adresse ,ab der gelesen werden soll'
    );
    ok(r3 && r3.lo === 0x80 && r3.hi === 0xdf, 'DPRAM address range 0x80-0xDF');
    ok(
      TM._parseRange('no range here') === null,
      'a comment with no range yields null'
    );
  }
  {
    const c = TM._parseCount(
      'Anzahl der 2-Byte-Worte (max. 16 Worte = 32 Bytes), die ausgelesen werden koennen'
    );
    ok(
      c.unit === 'word' && c.max === 16 && c.wordBytes === 2,
      'EEPROM counts 16 WORDS (2 bytes each), not 32 bytes'
    );
    const c2 = TM._parseCount(
      'Anzahl der Bytes (max. 32 !) die ausgelesen werden sollen '
    );
    ok(
      c2.unit === 'byte' && c2.max === 32 && c2.wordBytes === 1,
      'ROM counts 32 BYTES'
    );
    ok(
      c.unit !== c2.unit,
      'the two units differ -- the whole point of parsing them'
    );
  }
  ...
  console.log('region assembly from a real spec');
  {
    ...
    ok(
      r && r.lo === 0 && r.hi === 0xff && r.max === 16 && r.unit === 'word',
      'EEPROM_LESEN region parsed whole'
    );
    ok(
      r && r.order.join(';') === 'ADRESSE;BYTE_ANZAHL',
  ...
  }
  {
      ...
      arguments: [
        {
          ARG: 'RAM_TYPE',
          ARGTYPE: 'string',
          ARGCOMMENT0: '"INTERN" ,"EXTERN" oder "DP_RAM" ',
        },
        {
          ARG: 'ADRESSE',
          ARGTYPE: 'string',
          ARGCOMMENT0:
            'Hexwert (0x000-0xFFF) der Adresse ,ab der das Ram gelesen werden soll',
        },
        {
          ARG: 'BYTE_ANZAHL',
          ARGTYPE: 'int',
    ...
    ok(
      r && r.types.join(',') === 'INTERN,EXTERN,DP_RAM',
      'RAM_LESEN selector values parsed: ' + (r ? r.types.join(',') : '-')
    );
    ok(
      r && r.order[0] === 'RAM_TYPE',
      'selector stays first in the argument order'
    );
  }
  {
    ...
    ok(
      TM._regionFromArgs('STEUERN_LEUCHTE', spec) === null,
  ...
  console.log('DATEN parsing');
  {
    ...
    ok(
      a.length === 6 && a[0] === 0x01 && a[1] === 0xbf && a[5] === 0x80,
      'spaced hex string'
    );
...
`;
      ...
      ok(
        checked > 0,
        `parsed ${checked} region specs from the real E46 archive`
      );
    }
  }
  ...
  console.log('a refused read is reported, not silently empty');
  {
    ...
    apiStub = async () => ({
      sets: [{ JOB_STATUS: 'ERROR_ECU_CONDITIONS_NOT_CORRECT' }],
    });
    let threw = null;
    try {
    ...
    ok(
      threw !== null,
      'a non-OKAY JOB_STATUS throws rather than returning empty'
    );
    ok(
      threw && /CONDITIONS_NOT_CORRECT/.test(threw.message),
      "the error carries the ECU's own status text"
    );
    ok(
      threw && threw.arg === '0x00;16',
      'the failing argument is attached for display: ' + (threw && threw.arg)
    );

    // ...and an OKAY read still works through the same path
    apiStub = async () => ({
      sets: [{ JOB_STATUS: 'OKAY', DATEN: '01 BF 48 28' }],
    });
    const good = await TM.readChunk('kombi46', region, 0, 2);
    ok(
      good.bytes.length === 4 && good.bytes[1] === 0xbf,
      'an OKAY read returns its bytes'
    );
  }
  ...
  console.log("chunking walks the region in the job's own unit");
  {
    ...
    const { bytes } = await TM.readRange(
      'kombi46',
      region,
      region.lo,
      region.hi,
    ...
    ok(
      seen.length === 16,
      `256 words at 16/read = 16 calls (got ${seen.length})`
    );
    ok(
      seen[0] === '0x00;16' && seen[1] === '0x10;16',
      'addresses step by the chunk size in WORDS: ' +
        seen.slice(0, 2).join(', ')
    );
    ok(
      bytes.length === 512,
      `256 words x 2 bytes = 512 bytes (got ${bytes.length})`
    );
    ok(
      bytes[32] === 0x10,
      'chunk 2 landed at the right offset (word 16 -> byte 32)'
    );
  }
  ...
  console.log('a short answer ends the walk');
  {
    ...
    apiStub = async () => {
      n++;
      return {
        sets: [{ JOB_STATUS: 'OKAY', DATEN: n <= 2 ? '00 11 22 33' : '' }],
      };
    };
    const { bytes } = await TM.readRange(
      'kombi46',
