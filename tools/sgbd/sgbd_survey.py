            ...
            for e in sec.get("ecus", []):
                ...
                for stem in [e.get("sgbd")] + list(e.get("variants") or []):
                    ...
                    if stem.endswith(".prg"):
                        stem = stem[:-4]
                    # only what we can actually compile: the .prg must be
                    # present. have_prg, not a bare glob -- the vendor tree
                    # ships both *.prg and *.PRG and an exact-case match
                    # silently drops one spelling.
