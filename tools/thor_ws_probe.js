...
function raw(payload, ms = 3000) {
  return new Promise((resolve) => {
    ...
    sock.on('error', (e) => { rx = Buffer.from(`ERR ${e.message}`); finish(); });
  ...
  return new Promise((resolve) => {
    ...
    const give = (r) => {
      ...
      resolve({
        ...
        onData: (cb) => { onFrames = cb; },
      });
    };
    sock.setTimeout(ms);
    sock.on('data', (d) => {
      if (settled) {              // already upgraded: this is frame data
        frames.push(d);
...
function unframe(buf) {
  ...
  let len = buf[1] & 0x7f;
  let pos = 2;
  if (len === 126) { len = (buf[2] << 8) | buf[3]; pos = 4; }
...
(async () => {
  ...
  if (/this endpoint is a websocket/i.test(getTxt)) {
    ...
    console.log('   -> 404: the route is NOT in this firmware. Wrong image,\n'
  ...
  if (!hs.head) {
    ...
    console.log('   -> the ESP accepted the socket then went silent, which is\n'
              + '      what a crash in the handler looks like.\n');
    try { hs.sock.destroy(); } catch {}
    process.exit(1);
  }
  ...
  console.log(`   accept key ${okAccept ? 'correct' : 'WRONG'}`
  ...
  if (!rx.length) {
    ...
    console.log('   -> the socket is up but the UART is silent. Check the\n'
  ...
  const f = unframe(rx);
  if (!f) {
    console.log(`   ${rx.length} bytes, not a complete frame: ${hex(rx.slice(0, 24))}\n`);
    try { hs.sock.destroy(); } catch {}
    process.exit(1);
  }
  console.log(`   opcode 0x${f.op.toString(16)} `
  ...
  const ans = p.length >= tel.length + 9 ? p.slice(tel.length, tel.length + 9)
            : p.length >= 9 ? p.slice(0, 9) : null;
  if (ans && sum8(ans.slice(0, 8)) === ans[8]) {
    ...
    console.log('   -> the bridge works end to end. Point BMWeb at\n'
              + `      ws://${HOST}${PATH} and connect.\n`);
  } else {
    console.log('\n   bytes came back but they are not a valid ident.');
    console.log('   -> something answered; if the checksum is wrong the path\n'
