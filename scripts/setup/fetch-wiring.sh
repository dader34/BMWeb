...
set -euo pipefail
cd "$(dirname "$0")"
echo "note: fetch-wiring.sh is now fetch.sh --wiring -- running that" >&2
# --list is a mode, not a chassis name: pass it straight through
if [ "${1:-}" = "--list" ]; then exec ./fetch.sh --list; fi
exec ./fetch.sh --wiring "$@"
